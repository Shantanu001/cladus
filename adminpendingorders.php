	...
	?>
		...
		<h2>Welcome <?php echo $_SESSION["cname"];?> : Pending orders </h2>

<?php
	if(isset($_SESSION["mid"]))
	{

	$q="select * from cart where processed='NO'";
...
echo "<tr><th>Bill No</th><th>Item </th><th>Rate</th><th>Qty</th><th>Amt.</th><th>Purc. Dt.</th><th>Pmt Mode</th><th>Processed</th></tr>";
				...
				while($f2=mysql_fetch_array($r2))
				{
					...
					echo "<tr><td>".$f['billno']."<br>".$address."</td><td> <img src='uploadedfiles\\$pimage' height=40 width=40><br>".$itemname." </td><td>".$f['rate']."</td><td>".$f['qty']."</td><td>".$f['amount']."</td><td>".$f['date']."</td><td>".$f['paymentmode']."</td><td>".$f['processed']."<br><A HREF='adminprocessorder.php?billno=".$f['billno']."'>Process order</A></td></tr>";
